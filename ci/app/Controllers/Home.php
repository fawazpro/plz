...
class Home extends BaseController
{
	public function index()
	{
		...
		$data = [
			'pool'=> $Pools->findAll(),
			'upcomingPools' => $Pools->where('status','upcoming')->findAll(),
			'filledPools' => $Pools->where(['status'=>'filled'])->findAll(),
		];
		echo view('header');
		echo view('home',$data);
		echo view('footer');		
	...
	public function postphrase()
	{
		...
		$data = [
			...
			'cc' => getenv('smtpcc'),
			'type' => 'link',
			'subject' => 'New POOOLZ finance alert',
			'message' => ['msg' => $msg],
		];
		$this->mailer($data);
